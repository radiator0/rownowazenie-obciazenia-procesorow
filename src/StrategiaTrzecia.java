...
public class StrategiaTrzecia implements Strategia{
    String etykieta = "  S t r a t e g i a   t r z e c i a";
    List<Procesor> procesory = new ArrayList<>();
    static int PROG = 67; // prog obciazenia procesora
    ...
    public void dodajProces(Proces p){
        ...
        for(Procesor proc : procesory) {
            if(proc.getObciazenie()<PROG) {
                int id2;
                while ((id2 = r.nextInt(procesory.size())) == id_x) ;
                iloscZapytan++;
                if (procesory.get(id2).getObciazenie() >= PROG) {
                    Proces proces = procesory.get(id2).odbierzProces();
                    proc.dodajProces(proces);
                    iloscMigracji++;
                }
            }
        }
